...
static struct SIFTFeature* interp_extremum( IplImage***, int, int, int, int, int,
	double);
...
static double interp_contr( IplImage***, int, int, int, int, double, double,
	double );
static struct SIFTFeature* new_feature( void );
...
struct SIFTFeature * );
static struct SIFTFeature* clone_feature( struct SIFTFeature * );
...
static void interp_hist_entry( double***, double, double, double, double, int,
	int);
static void hist_to_descr( double***, int, int, struct SIFTFeature * );
static void normalize_descr( struct SIFTFeature * );
...
@see _sift_features()
*/
int sift_features( IplImage* img, struct SIFTFeature** feat )
{
...
@see sift_keypoints()
*/
int _sift_features( IplImage* img, struct SIFTFeature** feat, int intvls,
	...
	int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	octaveCount = log( (double)( MIN( init_img->width, init_img->height ) ) ) / log(2.0) - 2;
	gauss_pyr = build_gauss_pyr( init_img, octaveCount, intvls, sigma );
	dog_pyr = build_dog_pyr( gauss_pyr, octaveCount, intvls );

	storage = cvCreateMemStorage( 0 );
	features = scale_space_extrema( dog_pyr, octaveCount, intvls, contr_thr,
		curv_thr, storage );
	...
	n = features->total;
	*feat = (SIFTFeature*)calloc( n, sizeof(struct SIFTFeature) );
	*feat = (SIFTFeature*)cvCvtSeqToArray( features, *feat, CV_WHOLE_SEQ );
	for( i = 0; i < n; i++ )
	{
	...
	release_pyr( &gauss_pyr, octaveCount, intvls + 3 );
	release_pyr( &dog_pyr, octaveCount, intvls + 2 );
	return n;
}
	...
	int intvls, double sigma )
{
	...
	*/
	...
	for( o = 0; o < octvs; o++ )
		...
		{
			// 첫번째 octave의  첫번째 gauss image는 base image
			if( o == 0  &&  i == 0 )
				gauss_pyr[o][i] = cvCloneImage(base);

			/* base of new octvave is halved image from end of previous octave */

			// 그 다음부터 첫번째 gauss image는 이전 octave의 첫번째 image의 절반 크기
			else if( i == 0 )
				gauss_pyr[o][i] = downsample( gauss_pyr[o-1][intvls] );

			/* blur the current octave's last image to create the next one */
			// 나머지는 현재 octave의 첫번째 image에 gaussian smoothing
			else
			{
...
static IplImage*** build_dog_pyr( IplImage*** gauss_pyr, int octvs, int intvls )
{
	...
	for( o = 0; o < octvs; o++ )
		...
		{
			// gauss image의 k, k+1의 subtraction
	...
	CvMemStorage* storage )
{
	...
	features = cvCreateSeq( 0, sizeof(CvSeq), sizeof(struct SIFTFeature), storage );
	for( o = 0; o < octvs; o++ )
	{
		for( i = 1; i <= intvls; i++ )
		{
			for(r = SIFT_IMG_BORDER; r < dog_pyr[o][0]->height-SIFT_IMG_BORDER; r++)
			{
				for(c = SIFT_IMG_BORDER; c < dog_pyr[o][0]->width-SIFT_IMG_BORDER; c++)
				{
					...
					{
						// maximum 이거나 minimum 인 것들만 
						if( is_extremum( dog_pyr, o, i, r, c ) )
						{
							// accurate postion 으로 interpolation & DOG 값으로 filtering 
							feat = interp_extremum(dog_pyr, o, i, r, c, intvls, contr_thr);
							if( feat )
							{
								ddata = feat_detection_data( feat );

								// Harris corner response 값으로 corner point 같은 놈은 filtering
...
*/
static struct SIFTFeature* interp_extremum( IplImage*** dog_pyr, int octv,
	...
	double contr_thr )
{
	...
	{
		// 3차 함수의 근사 후 maximum point 찾기 (interpolation)
		...
		// 0.5 보다 크면 interpolation 방향으로 1 pixel 이동
		c += cvRound( xc );
		r += cvRound( xr );
		intvl += cvRound( xi );


		// 진행된 위치가 gaussian pyramid를 벗어나면 interpolation 실패
		// image margin 경계를 벗어나도 실패
		if( intvl < 1  ||
			intvl > intvls  ||
			c < SIFT_IMG_BORDER  ||
			r < SIFT_IMG_BORDER  ||
	...
	// 5 step 이상은 interpolation 하지 않음
	// 즉, extreme point 를 중심으로 포물선을 그리지 않고, 증가만 하면 interpolation이 안되는 거징
	if( i >= SIFT_MAX_INTERP_STEPS )
		return NULL;

	// interpolated point 에서의 DOG 값 계산 
	contr = interp_contr( dog_pyr, octv, intvl, r, c, xi, xr, xc );

	// thr 보다 작으면 filtering
	...
	// interpolated location, octave index, interval index, xi as subinterval
	ddata->r = r;
	ddata->c = c;
	ddata->octv = octv;
	ddata->intvl = intvl;
	...
	double* xi, double* xr, double* xc )
{
	...
	// f'(x) = y0 + a*x + 1/2*b*x2
	// 완전 제곱으로 변형 f'(x) = 1/2*b(x + a/b)2 + y0 - 1/2*a2/b완
	// Taylor 근사된 2차 식에서 극값의 위치는 -a/b 
	// 3차 함수에서는 - H2-1 x H1 가 됨 
	cvReleaseMat( &dD );
	cvReleaseMat( &H );
	cvReleaseMat( &H_inv );
...
static CvMat* deriv_3D( IplImage*** dog_pyr, int octv, int intvl, int r, int c )
{
	...
	// gaussian pyramid의 +1, -1 DOG 편차 /2  
	...
	int c, double xi, double xr, double xc )
{
	...
	// [dx, dy, ds]T 
	dD = deriv_3D( dog_pyr, octv, intvl, r, c );
	// [dx, dy, ds] x [x, y, s]T = dx * x + dy * y + ds + s
	cvGEMM( dD, &X, 1, NULL, 0, &T,  CV_GEMM_A_T );
	cvReleaseMat( &dD );
	// 찾아진 interpolated point 에서의 DOG (s) 값 계산 
...
static struct SIFTFeature* new_feature( void )
{
	...
	feat = (SIFTFeature*)malloc( sizeof( struct SIFTFeature ) );
	memset( feat, 0, sizeof( struct SIFTFeature ) );
...
static void calc_feature_scales( CvSeq* features, double sigma, int intvls )
{
	...
	for( i = 0; i < n; i++ )
	{
		feat = CV_GET_SEQ_ELEM( struct SIFTFeature, features, i );
		ddata = feat_detection_data( feat );
...
static void add_good_ori_features( CvSeq* features, double* hist, int n,
	double mag_thr, struct SIFTFeature* feat )
{
	struct SIFTFeature* new_feat;
...
*/
static struct SIFTFeature* clone_feature( struct SIFTFeature* feat )
{
	struct SIFTFeature* new_feat;
	struct detection_data* ddata;
	...
	memcpy( new_feat, feat, sizeof( struct SIFTFeature ) );
...
*/
static void hist_to_descr( double*** hist, int d, int n, struct SIFTFeature* feat )
{
...
@param feat feature
*/
static void normalize_descr( struct SIFTFeature* feat )
{
...
{
	struct SIFTFeature* f1 = (struct SIFTFeature*) feat1;
	struct SIFTFeature* f2 = (struct SIFTFeature*) feat2;
