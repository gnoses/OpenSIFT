...
static struct kd_node* kd_node_init( struct SIFTFeature *, int );
...
static struct kd_node* explore_to_leaf( struct kd_node*, struct SIFTFeature *,
					struct min_pq* );
static int insert_into_nbr_array( struct SIFTFeature *, struct SIFTFeature **, int, int );
...
/*
  ...
  @return Returns the root of a kd tree built from features or NULL on
    error.
*/
struct kd_node* kdtree_build( struct SIFTFeature* features, int n )
{
...
/*
  ...
  @return Returns the number of neighbors found and stored in nbrs, or
    -1 on error.
*/
int kdtree_bbf_knn( struct kd_node* kd_root, struct SIFTFeature* feat, int k,
		    struct SIFTFeature*** nbrs, int max_nn_chks )
{
  struct kd_node* expl;
  struct min_pq* min_pq;
  struct SIFTFeature* tree_feat, ** _nbrs;
  ...
  _nbrs = (SIFTFeature**)calloc( k, sizeof( struct SIFTFeature* ) );
  min_pq = minpq_init();
...
*/
int kdtree_bbf_spatial_knn( struct kd_node* kd_root, struct SIFTFeature* feat,
			    int k, struct SIFTFeature*** nbrs, int max_nn_chks,
			    CvRect rect, int model )
{
  struct SIFTFeature** all_nbrs, ** sp_nbrs;
  CvPoint2D64f pt;
  int i, n, t = 0;

  n = kdtree_bbf_knn( kd_root, feat, max_nn_chks, &all_nbrs, max_nn_chks );
  sp_nbrs = (SIFTFeature**)calloc( k, sizeof( struct SIFTFeature* ) );
  for( i = 0; i < n; i++ )
    {
      if( model )
...
*/
static struct kd_node* kd_node_init( struct SIFTFeature* features, int n )
{
...
{
  struct SIFTFeature* features, tmp;
  double kv;
  int n, ki, p, i, j = -1;
...
/*
  ...
  @return If feat was successfully inserted into nbrs, returns 1; otherwise
    returns 0.
*/
static int insert_into_nbr_array( struct SIFTFeature* feat, struct SIFTFeature** nbrs,
				  int n, int k )
{
